...
void USART3_IRQHandler(void)
{
	...
	{	 
		...
		if((USART3_RX_STA&0x8000)==0)//接收未完成
		{
			...
			USART3_RX_STA++;
			if(USART3_RX_STA>(USART3_MAX_SEND_LEN-1)) //接收数据错误,重新开始接收	  
				USART3_RX_STA=0;
		}
	}		
}
...
//		modbus_time_ms++;
//		if(modbus_time_ms>15) modbus_time_ms=15;
...
{
	Usart_SendStrLen(USART3,buff,len);
}
...
void modbus_service(void)
{
	...
	if(USART3_RX_STA&0x8000)   //串口接收完成的标志
	{
		
		data_len = USART3_RX_STA & 0x3fff ; 	
		
		CRC_check_result = CRC16(USART3_RX_BUF,data_len-2);
		data_CRC_value= USART3_RX_BUF[data_len-1]<<8 | (((u16)USART3_RX_BUF[data_len-2])); //对于CRC的接收，先接收低位，再接收高位
		
//		modbus_send_data(USART3_RX_BUF,data_len);
		if(CRC_check_result==data_CRC_value)
		{
			if(USART3_RX_BUF[0]==modbus_slave_addr)
			{
...
void modbus_03_function(void)
{
	...
	modbus_Tx_buff[1] = 03;
	register_len = USART3_RX_BUF[5]|(((u16)USART3_RX_BUF[4])<<8); 
	modbus_Tx_buff[2] = register_len * 2;
	register_start = USART3_RX_BUF[3]|(((u16)USART3_RX_BUF[2])<<8);
...
void modbus_06_function(void)
{
	...
	modbus_Tx_buff[0]=USART3_RX_BUF[0];
	modbus_Tx_buff[1]=USART3_RX_BUF[1];
	modbus_Tx_buff[2]=USART3_RX_BUF[2];
	modbus_Tx_buff[3]=USART3_RX_BUF[3];
	modbus_Tx_buff[4]=USART3_RX_BUF[4];
	modbus_Tx_buff[5]=USART3_RX_BUF[5];
	
	register_addr = USART3_RX_BUF[3]|(((u16)USART3_RX_BUF[2])<<8);
	modbus_virtual_register[register_addr] = USART3_RX_BUF[5]|(((u16)USART3_RX_BUF[4])<<8);
...
void modbus_16_function(void)
{
	...
	register_start = USART3_RX_BUF[3]|(((u16)USART3_RX_BUF[2])<<8);  
	register_num = USART3_RX_BUF[5]|(((u16)USART3_RX_BUF[4])<<8);  
	for( i =0;i<register_num;i++)
	{
		modbus_virtual_register[register_start+i] = USART3_RX_BUF[8+i*2]|(((u16)USART3_RX_BUF[7+i*2])<<8);
	}
	modbus_send_data(modbus_Tx_buff,8);
